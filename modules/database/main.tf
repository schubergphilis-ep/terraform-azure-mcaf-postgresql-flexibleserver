...
module "entra_rbac_reader" {
  source          = "../rbac_entra"
  for_each        = { for user_object in var.readers : coalesce(user_object.display_name, user_object.name, user_object.object_id, user_object.principal_id) => user_object }
  database_name   = azurerm_postgresql_flexible_server_database.this.name
  role_name       = "readers"
...
module "entra_rbac_writer" {
  source          = "../rbac_entra"
  for_each        = { for user_object in var.writers : coalesce(user_object.display_name, user_object.name, user_object.object_id, user_object.principal_id) => user_object }
  database_name   = azurerm_postgresql_flexible_server_database.this.name
  role_name       = "writers"
  role_assignment = each.value
  roles           = ["pg_read_all_data", "pg_write_all_data"]
...
module "entra_rbac_admin" {
  source          = "../rbac_entra"
  for_each        = { for user_object in var.admins : coalesce(user_object.display_name, user_object.name, user_object.object_id, user_object.principal_id) => user_object }
  database_name   = azurerm_postgresql_flexible_server_database.this.name
  role_name       = "admins"
  role_assignment = each.value
  roles           = ["pg_read_all_data", "pg_write_all_data"]
